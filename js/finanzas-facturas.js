document.addEventListener('DOMContentLoaded', () => {
  ...
  const btnTabFacturas = document.querySelector('.subtab-btn[data-tab="facturas"]');

  // Solo admin/control ven esta pestaña (asistente_contable no, mismo criterio que tenía dashboard.html)
  if (rol !== 'admin' && rol !== 'control') {
    if (btnTabFacturas) btnTabFacturas.style.display = 'none';
    return;
  }
  ...
  // DOM
  // =========================
  const loader = document.getElementById('loader-facturas');
  const contenido = document.getElementById('contenido-facturas');
  ...
  let facturas = [];
  let paginaActual = 1;
  let _facturasCargado = false;
  let _loaderTimer = null;

  // =========================
  // Carga perezosa: solo la primera vez que se abre la pestaña Facturas
  // =========================
  if (btnTabFacturas) {
    btnTabFacturas.addEventListener('click', () => {
      if (!_facturasCargado) {
        _facturasCargado = true;
        init();
      }
    });
  }
  ...
  // Muestra el loader solo si la carga tarda más de los ms indicados.
